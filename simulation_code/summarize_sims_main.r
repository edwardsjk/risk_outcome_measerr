## Code to summarize main simulations ##
...
df <-
  list.files(path = "final_code/sims/main_paper/r1/output/s200", pattern = "\\.csv", 
  full.names = T) %>% 
  map_df(~read_csv(.))
df

df2 <-
  list.files(path = "final_code/sims/main_paper/r1/output/s2500", pattern = "\\.csv", 
  full.names = T) %>% 
  map_df(~read_csv(.)) %>% 
...
# read in all data ----
dfraw <-
  list.files(path = "final_code/sims/main_paper/r1/output/raw200", pattern = "\\.csv", 
  full.names = T) %>% 
  map_df(~read_csv(.))
dfraw

dfraw2 <-
  list.files(path = "final_code/sims/main_paper/r1/output/raw2500", pattern = "\\.csv", 
  full.names = T) %>% 
  map_df(~read_csv(.))
dfraw2
...
ggsave(plotmat, file = "final_code/sims/main_paper/r1/output/bothpl.pdf", width = 25, height = 20, units = "cm")
