## Code to summarize simulations with validation study with different shape param ##
...
# df <-
#   list.files(path = "final_code/sims/main_paper/r1/output/alts200", pattern = "\\.csv", 
#   full.names = T) %>% 
#   map_df(~read_csv(.))
# df
# 
# df2 <-
#   list.files(path = "final_code/sims/main_paper/r1/output/alts2500", pattern = "\\.csv", 
#   full.names = T) %>% 
#   map_df(~read_csv(.)) %>% 
#   select(bias, ese, rmse, ase, ser, cov)
# df2
# 
...
# # write csv ----
# 
# write_csv(df3, file = "final_code/sims/main_paper/r1/output/alltab.csv")

# read in all data ----
dfraw <-
  list.files(path = "final_code/sims/main_paper/r1/output/altraw200", pattern = "\\.csv", 
  full.names = T) %>% 
  map_df(~read_csv(.))
dfraw

dfraw2 <-
  list.files(path = "final_code/sims/main_paper/r1/output/altraw2500", pattern = "\\.csv", 
  full.names = T) %>% 
  map_df(~read_csv(.))
dfraw2
...
ggsave(plotmat, file = "final_code/sims/main_paper/r1/output/bothpl_altval.pdf", width = 25, height = 20, units = "cm")
