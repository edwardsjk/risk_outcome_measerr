...
### Repeat sims with validation dataset with incorrect params (nval = 2500) ###
...
res <- foreach(s=1:J, .combine='rbind',
             .packages = c("survival", "tidyr", "dplyr", "doParallel", "resample")) %dopar%{
               wrap(s, randos[s], taus, B, rmst = T, perfectval = F)
             }
...
write.csv(results, file = paste0("output/wv_server2500", scenario, "_", today, ".csv"))
...
write.csv(sc_res, file = paste0("output/wv_tab2500_", today, "_", scenario, ".csv"))
